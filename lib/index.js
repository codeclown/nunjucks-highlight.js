module.exports = function NunjucksCodeHighlight (nunjucks, hljs) {
    ...
    this.blockTag = function (environment, blockTagBody, blockTagTabStart) {

        var body = blockTagBody();
        var spacesRegex = /^[\s]+/;
        var tabStart = blockTagTabStart(); // The column postion of the {% code %} tag.
        ...
        if (tabStart > 0) { // If the {% code %} tag is tabbed in, normalize the content to the same depth.
            ...
            // remove blank start and end lines to be safe
            if (body[0] === "")
                body = body.slice(1);
